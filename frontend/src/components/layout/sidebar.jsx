...
export default function LeftSidebar({ entryCount }) {
  ...
  useEffect(() => {
    const fetchLastScreening = async () => {
      try {
        ...
        const res = await fetch(`${import.meta.env.VITE_API_URL}/screening/history`, {
    ...
    fetchLastScreening();
  }, [location.pathname]); // refetch saat navigasi berubah

  // Hitung sisa hari sampai bisa screening lagi (7 hari cooldown)
  const getDaysRemaining = () => {
    if (!lastScreening?.createdAt) return null;
    const lastDate = new Date(lastScreening.createdAt);
  ...
  const displayCount = entryCount ?? screeningCount;
  ...
  return (
    <aside className="journal-sidebar">
      ...
      <div className="sidebar-stat-card">
        ...
        <div className="stat-card-value">{displayCount}</div>
        <div className="stat-card-sub">dari 30 hari</div>
        <div className="stat-bar">
          <div
            className="stat-bar-fill"
            style={{ width: `${Math.min((displayCount / 30) * 100, 100)}%` }}
          />
        </div>
      </div>
      ...
      <div className="sidebar-screening-card">
        {lastScreening ? (
          <>
            <div className="screening-date">{formatDate(lastScreening.createdAt)}</div>
            <div className="screening-result">{lastScreening.level}</div>
            <div className="screening-note">
              Skor {lastScreening.total_score}
          ...
          <>
            <div className="screening-date">Belum ada data</div>
            <div className="screening-note">Lakukan screening pertamamu</div>
          </>
        )}
      </div>
    </aside>
